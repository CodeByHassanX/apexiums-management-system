...
async function main() {
    ...
    const createdPermissions = await Promise.all(defaultPermissions.map(action => prisma.permission.upsert({
        where: { action },
        update: {},
        create: { action },
    })));
    ...
    if (superAdminRole) {
        ...
        const hashedManagerPwd = await bcryptjs_1.default.hash('manager123', 10);
        await prisma.user.upsert({
