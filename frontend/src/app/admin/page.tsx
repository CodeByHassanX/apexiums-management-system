  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between group hover:border-[#12b4a3]/30 transition-colors">
          <div>
            ...
            <div className="text-4xl font-black text-gray-900 mt-2">
              Rs {loading ? '-' : stores.reduce((acc, s) => acc + (Number(s.monthlyRent) || 0), 0).toLocaleString()}
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between group hover:border-[#12b4a3]/30 transition-colors">
          <div>
            <div className="text-sm font-bold text-gray-500 uppercase tracking-wider">Total Stores</div>
            <div className="text-4xl font-black text-gray-900 mt-2">{loading ? '-' : stores.length}</div>
          </div>
          <div className="h-14 w-14 rounded-full bg-[#e2f9f6] flex items-center justify-center text-[#12b4a3] group-hover:scale-110 transition-transform">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between group hover:border-[#12b4a3]/30 transition-colors">
          <div>
            <div className="text-sm font-bold text-gray-500 uppercase tracking-wider">Active Portal login</div>
            <div className="text-4xl font-black text-gray-900 mt-2">{loading ? '-' : stores.filter(s => s.status === 'ACTIVE').length}</div>
          </div>
          <div className="h-14 w-14 rounded-full bg-[#e2f9f6] flex items-center justify-center text-[#12b4a3] group-hover:scale-110 transition-transform">
        ...
        {/* system health */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between group hover:border-green-500/30 transition-colors">
          <div>
          ...
          <div className="h-14 w-14 rounded-full bg-green-50 flex items-center justify-center text-green-500 group-hover:scale-110 transition-transform">
